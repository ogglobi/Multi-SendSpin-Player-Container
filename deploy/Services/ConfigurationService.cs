...
public class PlayerConfiguration
{
    ...
    /// <summary>
    /// Per-player volume scale factor (0.01-1.0). Multiplied with global volume scale.
    /// Use this to reduce maximum loudness for this specific player. Default is null (use global scale).
    /// Example: Volume 50% × GlobalScale 0.5 × PlayerScale 0.6 = 15% effective volume
    /// </summary>
...
/// <summary>
/// Global configuration settings that apply to all players.
/// Stored in a separate global.yaml file.
/// </summary>
public class GlobalConfiguration
{
    /// <summary>
    /// Global volume scale factor (0.01-1.0). Applied to all players as a multiplier.
    /// Use this to reduce maximum loudness system-wide. Default is 1.0 (no reduction).
    /// Example: Volume 50% × GlobalScale 0.5 = 25% effective volume
    /// </summary>
...
public class ConfigurationService
{
        ...
        EnvironmentService environment)
    {
        ...
        _globalConfigPath = Path.Combine(environment.ConfigPath, "global.yaml");

        _logger.LogDebug("Initializing ConfigurationService with players config: {PlayersPath}, devices config: {DevicesPath}, global config: {GlobalPath}",
            _playersConfigPath, _devicesConfigPath, _globalConfigPath);
    ...
    /// <summary>
    /// Global volume scale factor (convenience property).
    /// </summary>
    public decimal GlobalVolumeScale => _global.GlobalVolumeScale;
    ...
    {
        _logger.LogDebug("Loading global configuration from {ConfigPath}", _globalConfigPath);

        try
        {
            if (!File.Exists(_globalConfigPath))
            {
                _logger.LogDebug("Global config file does not exist, using defaults");
            ...
            if (string.IsNullOrWhiteSpace(yaml))
            {
                _logger.LogDebug("Global config file is empty, using defaults");
                _global = new GlobalConfiguration();
                return;
            }

            var loaded = _deserializer.Deserialize<GlobalConfiguration>(yaml);
            _global = loaded ?? new GlobalConfiguration();

            // Clamp to valid range
            if (_global.GlobalVolumeScale < 0.01m)
                _global.GlobalVolumeScale = 0.01m;
            if (_global.GlobalVolumeScale > 1.0m)
                _global.GlobalVolumeScale = 1.0m;

            _logger.LogInformation("Loaded global configuration: GlobalVolumeScale={VolumeScale}",
                _global.GlobalVolumeScale);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to load global configuration from {ConfigPath}", _globalConfigPath);
            _global = new GlobalConfiguration();
        }
    }
    ...
    public bool SaveGlobal()
    {
        try
        {
            var yaml = _serializer.Serialize(_global);
            File.WriteAllText(_globalConfigPath, yaml);
            _logger.LogInformation("Saved global configuration: GlobalVolumeScale={VolumeScale}",
                _global.GlobalVolumeScale);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to save global configuration to {ConfigPath}", _globalConfigPath);
            return false;
        }
    }

    /// <summary>
    /// Set the global volume scale (0.01-1.0).
    /// </summary>
    public bool SetGlobalVolumeScale(decimal scale)
    {
        scale = Math.Clamp(scale, 0.01m, 1.0m);
        _global.GlobalVolumeScale = scale;
        return SaveGlobal();
    }
